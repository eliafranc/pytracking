...
def run_tensor_dataset(dataset, trackers, delta_t=10, rgb_only=False, debug=False, threads=0, visdom_info=None):
    """Runs a list of trackers on a dataset.
    args:
        dataset: List of Sequence instances, forming a dataset.
        trackers: List of Tracker instances.
    ...
    if mode == "sequential":
        ...
        param_list = [
            (seq, tracker_info, delta_t, rgb_only, debug, visdom_info)
            for seq, tracker_info in product(dataset, trackers)
        ]
        with multiprocessing.Pool(processes=threads) as pool:
            pool.starmap(run_tensor_sequence, param_list)
    print("Done")


def _build_sequence_dict(seq):
